...
- **Requirement**: At the conclusion of processing every user prompt/task (after completing all code modifications, verification, and the silent version increment), every AI agent MUST execute the following sequence:
  ...
  2. `git commit -m "<concise descriptive summary of changes>"` (commit with a clear message detailing what happened during the prompt)
...
- **Lifecycle Sequence at End of Prompt**:
  ...
  3. Git Stage: `git add .`
  4. Git Commit: `git commit -m "what happens here"` (substituting with a concise description of actual changes)
  5. Git Push: `git push`
