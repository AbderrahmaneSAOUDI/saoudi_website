...
18. **Mandatory End-of-Prompt Git Automation:** At the conclusion of every prompt, stage all changes (`git add .`), commit with a message starting with "updated project version" (`git commit -m "updated project version: <what happens here>"`), and push to the remote repository (`git push`).
