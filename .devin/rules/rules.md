...
You must strictly obey these 18 individual engineering and design rules for every single file modification, code generation, and workspace interaction.
...
18. **Mandatory End-of-Prompt Git Automation:** At the conclusion of every prompt, stage all changes (`git add .`), commit with a message starting with the project version from `package.json` (`git commit -m "<version> - <type>: <description>"`, e.g. `git commit -m "1.74.349 - fix: description here"`), and push to the remote repository (`git push`).
